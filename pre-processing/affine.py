# This is a modification of https://github.com/codingfisch/torchreg/blob/db0bc68ef17e61f2d5ed29e78fc271c2d49b9587/torchreg/affine.py
...
def init_parameters(is_3d=True, batch_size=1, device='cpu', with_translation=True, with_rotation=True, with_zoom=True,
    ...
    rotation = torch.zeros(batch_size, 3 if is_3d else 1) if init_rotation is None else init_rotation
...
def compose_affine(translation, rotation, zoom, shear):
    print(f"{translation}, {rotation}, {zoom}, {shear}")
    ...
    rigid_matrix = torch.stack([
        torch.stack([cos_theta, -sin_theta, 0]),
        torch.stack([sin_theta, cos_theta, 0]),
        torch.stack([0,0,0])
    ])

    return torch.cat([rigid_matrix, translation[:, :, None]], dim=-1)
...
def _check_parameter_shapes(translation, rotation, zoom, shear, is_3d=True, batch_size=1):
    ...
    for name, param in params.items():
        if param is not None:
            desired_shape = (batch_size, 3 if is_3d else 1) if name == 'rotation' else (batch_size, n_dim)
